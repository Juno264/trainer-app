...
2. メモ欄の内容を必ず分析に反映する
3. 種目の実施順を見て、後半種目の未達は「疲労」と判断する
4. 肯定から入り、改善点は1つに絞り、励ましで締める
5. 任意（bonus）種目が未実施でも責めない
...
- 2026-07-25 より前の `achievement_rate` は旧ロジックの値。再計算はしないため、
  グラフで推移を出すときは境界として扱うこと。履歴画面では「(旧基準)」と付記している。

### 重量・レップ数の調整は外部AI分析が行う（アプリからは書き込まない）

`/api/complete` は **`exercises.target_weight_kg` / `target_reps` を更新しない。**
セッション完了時のレビューは「今日の記録の要約」までが守備範囲で、
次にどの重量を扱うかの判断は外部のAI分析に一本化した。

- かつては自動調整していた。記録が0件の種目まで対象にしていたため未実施種目の
  目標重量が下がり続ける不具合があり（脚・お尻が57.5kgまで低下）、
  「実施した core 種目に限る」というガードを入れた。それでも
  **メモに書かれた器具の制約を無視した実在しない重量の指示**や、
  **自己ベストを事故と誤認した巻き戻し**が残ったため、判断そのものを外に出した。
- したがってプロンプトでは重量・レップ数の処方を**明示的に禁止**している。
  「次回は◯kgに」のような数値の指示を復活させないこと。
- `reviews.next_instruction` / `weight_adjustment_memo` は外部AI分析の書き込み先。
  **upsert の対象に含めないこと**（upsert は渡したキーだけをUPDATEするため、
  省略すれば外部AIが書いた内容がそのまま残る）。
- ユーザーへの指示は `exercises.instruction` に入れる。トレーニング画面の種目カードに
  **常時表示**する（折り畳みの中に入れると読まれないまま実施され、事故になる）。
  `adjustment_reason` はユーザー向け画面に出さない。
